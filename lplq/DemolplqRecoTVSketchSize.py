# we test the choice of sketch size in this file.
# test on Gaussian blur, p = 0.5, noise level 5%, leaves image
...
parser.add_argument('-MaxIter_Test', dest='MaxIter_Test', help='Maximal number of iterations for choosing the trade-off parameter',type=int,default = 5)
...
parser.add_argument('-sketch_size', dest='sketch_size', help='The size of sketch for building preconditioner: large-> better but more computational time',type=int,default = 300)
...
parser.add_argument('-im_task', dest='im_task', type=str,help='Image task: Deblur, SR, Denoise',default = 'Deblur') #  'SR'
...
parser.add_argument('-imName', dest='imName', type=str,help='Name of the tested image',default = 'leaves.tif')
...
parser.add_argument('-p', dest='p',help='The p norm of the data-fidelity part', type=float,default = 0.5)
...
# add impulse noise
b_noisy = utl.add_salt_and_pepper_noise(b,salt_prob,pepper_prob,isColor=isColor)
b_noisy_temp = b_noisy
...
beta = 0.003105410821643

algName = '/Pre_l' +str(p) + '_l' +str(q)+imName_only+'_salt'+str(salt_prob)+'pepp'+str(pepper_prob)+'Sketch'+str(sketch_size)
...
x_pre,lst_cost_pre,psnr_set_FISTA_TV_pre,\
    ssim_set_FISTA_TV_pre,lst_time_pre,lst_iter_pre = \
        opt.IRN_lplq(MaxIter,Ax,ATx,Phix,PhiTx,b_noisy_input,p,q,sketch_size=sketch_size,isPre=True,PhiTxBatch=PhiTxBatch,TR_off = beta,\
